...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setError(Array.isArray(e) ? e.map(i => i.message).join(', ') : e || t('admin.login.invalidCredentials'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gray-50">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-gray-900">{t('admin.login.title')}</h1>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-8 py-8">
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700">{t('admin.login.phoneNumber')}</label>
